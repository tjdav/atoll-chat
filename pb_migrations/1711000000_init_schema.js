...
migrate((app) => {
  ...
  const rooms = new Collection({
    ...
    fields: [
      ...
      new FileField({ name: "avatar", maxSelect: 1 }),
    ],
  });
  app.save(rooms);
  ...
  const messages = new Collection({
    ...
    createRule: "@request.auth.id != ''",
    fields: [
      new RelationField({ name: "room_id", collectionId: rooms.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "sender_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new TextField({ name: "payload", required: true }),
  ...
  const roomMembers = new Collection({
    ...
    fields: [
      new RelationField({ name: "room_id", collectionId: rooms.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "user_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new SelectField({ name: "status", required: true, maxSelect: 1, values: ["invited", "joined", "left"] }),
      new TextField({ name: "encrypted_room_key", required: true }),
      new RelationField({ name: "last_read_message_id", collectionId: messages.id, maxSelect: 1 }),
    ],
  });
  ...
  const reactions = new Collection({
    ...
    fields: [
      new RelationField({ name: "message_id", collectionId: messages.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "user_id", collectionId: users.id, required: true, maxSelect: 1 }),
  ...
  const trustedContacts = new Collection({
    ...
    deleteRule: "@request.auth.id != ''",
    fields: [
      new RelationField({ name: "owner_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "trusted_user_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new TextField({ name: "public_key_fingerprint", required: true }),
    ],
  });
  ...
  const callSignals = new Collection({
    ...
    fields: [
      new RelationField({ name: "room_id", collectionId: rooms.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "sender_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new RelationField({ name: "target_id", collectionId: users.id, required: true, maxSelect: 1 }),
      new TextField({ name: "signal_data", required: true }),
    ],
  });
