...
| `ATOLL_APP_URL` | **`[BUILD & RUNTIME]`** | None | Absolute URL pointing to the frontend web application (used by background cryptographic workers, redirects, and initial invite emails). | [coralite.config.js](/coralite.config.js), [pocketbasePlugin](/src/plugins/pocketbase.js), [cryptoPlugin](/src/plugins/crypto-worker.js), [initial_invite.pb.js](/database/pb_hooks/initial_invite.pb.js) |
