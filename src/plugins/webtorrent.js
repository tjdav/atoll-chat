...
export default function ({ trackerUrl = 'ws://localhost:8000' } = {}) {
  return createPlugin({
      ...
      helpers: {
        seed: (globalContext) => (localContext) => {
          ...
          return (file) => {
            return new Promise((resolve, reject) => {
              try {
                client.seed(file, { announce: [globalContext.config.trackerUrl] }, (torrent) => {
        ...
        download: (globalContext) => (localContext) => {
          ...
          return (magnetURI, onProgress) => {
            return new Promise((resolve, reject) => {
              try {
                client.add(magnetURI, { announce: [globalContext.config.trackerUrl] }, (torrent) => {
