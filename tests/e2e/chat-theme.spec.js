...
test.describe('Chat View Theme System & Component Verification E2E', () => {
  ...
  test('should apply themes and verify atoll-chat-view header, atoll-chat-input, file card, and date separator component styling uniformity', async ({ page }) => {
    ...
    await page.getByTestId('fileInput').setInputFiles(docPath)
    ...
    const roomSettingsBtn = page.getByTestId('btnRoomSettings')
    ...
    await page.getByTestId('btnChangeTheme').click()
    await expect(themeModal).toBeVisible()
    await page.getByTestId('theme-forest-item').click()
    ...
    await page.getByTestId('btnChangeTheme').click()
    await expect(themeModal).toBeVisible()
    await page.getByTestId('theme-classic-item').click()
  ...
  test('should allow user to upload custom image, generate palette, adjust sliders, and persist custom theme', async ({ page }) => {
    ...
    // Select Custom Theme
    await page.getByTestId('theme-custom-item').click()

    const customControls = page.getByTestId('custom-theme-controls')
    await expect(customControls).toBeVisible()

    const btnGenerate = customControls.getByTestId('btn-generate-palette')
    const btnRandomize = customControls.getByTestId('btn-randomize-colors')
    ...
    const slidersContainer = customControls.getByTestId('bg-sliders-container')
    await expect(slidersContainer).toBeVisible()

    const blurSlider = customControls.getByTestId('blur-slider')
    await blurSlider.fill('15')
    const dimSlider = customControls.getByTestId('dim-slider')
