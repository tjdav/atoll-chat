...
const THEMES = [
  ...
  { id: 'classic-dark', selectorId: 'classic', isDark: true, name: 'Classic Dark' },
  { id: 'ocean', name: 'Ocean' },
...
test.describe('Automated Accessibility (axe-core) & Theme Matrix Audits', () => {
  for (const theme of THEMES) {
    test(`should pass WCAG 2.1/2.2 AA & color-contrast audit for ${theme.name} theme`, async ({ browser, loginCustomPage }) => {
      test.setTimeout(120000)

      // 1. Setup dual browser contexts for Alice and Bob
      ...
      // 2. Alice creates a chat room with Bob
      ...
      await expect(alicePage.locator('chat-attachment-preview .attachment-preview-status')).toContainText('Ready to send', { timeout: 30000 })
      await alicePage.locator('[data-testid$="sendButton"]').click()
      await expect(alicePage.locator('message-timeline .waveform-player').first()).toBeVisible({ timeout: 30000 })
      ...
      await expect(alicePage.locator('chat-attachment-preview .attachment-preview-status')).toContainText('Ready to send', { timeout: 30000 })
      await alicePage.locator('[data-testid$="sendButton"]').click()
      await expect(alicePage.locator('message-timeline .file-attachment-container').first()).toBeVisible({ timeout: 30000 })
      ...
      // 3. Open Room Details Offcanvas & Theme Selector Modal
      const roomSettingsBtn = alicePage.locator('[ref$="btnDetails"] button')
