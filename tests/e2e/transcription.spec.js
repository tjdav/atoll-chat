...
test.describe('Voice Message Transcription E2E', () => {
  ...
  test('should transcribe a voice message successfully and show transcript', async ({ page }) => {
    ...
    const transcribeBtn = page.getByTestId('btn-transcribe').last()
  ...
  test('should not reset previously transcribed voice messages when transcribing a new one', async ({ page }) => {
    ...
    const firstTranscribeBtn = page.getByTestId('btn-transcribe').first()
    ...
    const secondTranscribeBtn = page.getByTestId('btn-transcribe').last()
    await expect(page.getByTestId('btn-transcribe')).toHaveCount(2, { timeout: 30000 })
