...
test.describe('Secure Calls', () => {
  ...
  test('Ringing State', async () => {
    ...
    await alicePage.locator('#coralite-chat-window__startVideoCallBtn-0').click()

    // Verify User B sees the incoming call modal/toast
    const incomingCallModal = bobPage.locator('#coralite-app-layout__incomingCallModal-0')
  ...
  test('Stream Connection', async () => {
    // User B accepts
    const acceptButton = bobPage.locator('#coralite-app-layout__acceptCallBtn-0')
    await acceptButton.click()

    // Verify the <coralite-video-call> modal opens for both users
    await expect(alicePage.locator('#coralite-video-call__callModal-0')).toBeVisible({ timeout: 10000 })
    await expect(bobPage.locator('#coralite-video-call__callModal-0')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Media Tracks Assigned', async () => {
    ...
    const checkVideoTracks = async (page) => {
      return page.evaluate(() => {
        ...
        const localVideo = document.querySelector('video[id^="coralite-video-call__localVideo"]')
        const remoteVideo = document.querySelector('video[id^="coralite-video-call__remoteVideo"]')
        return {
  ...
  test('Hangup and Cleanup', async () => {
    ...
    const endCallButton = alicePage.locator('#coralite-video-call__endCallBtn-0')
    await endCallButton.click()

    // Verify the modal closes for both users
    await expect(alicePage.locator('#coralite-video-call__callModal-0')).toBeHidden({ timeout: 5000 })
    await expect(bobPage.locator('#coralite-video-call__callModal-0')).toBeHidden({ timeout: 5000 })
